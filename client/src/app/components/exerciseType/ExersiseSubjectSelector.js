  ...
  return (
    <div className='exersiseTypeSelector-container'>
		...
		{types && types.map((type, i)=>{
			return <div onClick={()=>{const name = type.toLowerCase().split(' ').join('_'); console.log(name); localStorage.setItem('exersiseSubject', name); onClick(name)}} className={`exersiseTypeSelector--card ${type.toLowerCase().split(' ').join('_')}`} key={i}>{type}</div>
		})}
    </div>
  );
};
